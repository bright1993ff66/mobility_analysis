...
# Specify the color code for the tweets posted before and during the Covid19
before_color_code = '#1597A5'
during_color_code = '#FFC24B'
...
def plot_inertias(inertia_dict, highest_cluster_num: int = 20,
                  line_color: str = before_color_code,
                  save_filename: str = 'select_user_cluster_num.png') -> None:
    """
    ...
    :param line_color: the color of the line in the graph
    ...
    axis.plot(cluster_nums, inertia_vals, color=line_color, marker='x', linestyle='--')
...
def plot_geo_points_over_polygons(city_shape: GeoDataFrame,
    ...
    tweet_before_shape.plot(ax=axis, color=before_color_code, markersize=15, zorder=2, alpha=0.5,
                            label='Before Pandemic', marker='^')
    tweet_during_shape.plot(ax=axis, color=during_color_code, markersize=15, zorder=3, alpha=0.5,
...
def plot_footprint_mobility_cluster(mobility_metric_dataframe: pd.DataFrame,
                                    color_code: str = before_color_code) -> None:
    """
...
def create_plot_dist_time(mobility_data_before_covid: pd.DataFrame,
    ...
    define_box_properties(boxplot_before, before_color_code, 'Before Covid', fontsize=20)
    define_box_properties(boxplot_during, during_color_code, 'During Covid', fontsize=20)
...
def create_plot_tweet_metrics(mobility_data_before_covid: pd.DataFrame,
    ...
    # setting colors for each groups
    define_box_properties(boxplot_tweet_num_before, before_color_code, 'Before Covid')
    define_box_properties(boxplot_tweet_num_during, during_color_code, 'During Covid')
    define_box_properties(boxplot_time_span_before, before_color_code, 'Before Covid', fontsize=10)
    define_box_properties(boxplot_time_span_during, during_color_code, 'During Covid', fontsize=10)
